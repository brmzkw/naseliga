...
export const Leaderboard: React.FC = () => {
  ...
  const searchIdx =
    query.data?.findIndex((event) => event.id === selectedEvent?.id) ?? 0;
  ...
  return (
    <div>
      <div className="mb-2 flex justify-around p-2 shadow-md shadow-slate-900">
        <div>
          {prevEvent && (
        ...
        <div className="flex-1 text-center">
          {currentEvent && (
            <Title event={currentEvent} isFirst={currentIdx === 0} />
          )}
        </div>
        <div>
          {nextEvent && (
...
const Title: React.FC<TitleProps> = ({ event, isFirst }) => {
  const eventDate = `${event.date.toLocaleString("default", {
    month: "short",
...
const LeaderboardContent: React.FC<LeaderboardContentProps> = ({ event }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-2 self-end rounded-xl bg-slate-700 p-2 text-xs text-white hover:bg-slate-500">
