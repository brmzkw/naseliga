  ...
  return (
    <div>
      <table className="m-auto">
        <tbody>
          {event.matches.map((match) => (
            <tr
              key={match.id}
              className={`border-b border-purple-700 [&>td]:p-3`}
            >
              <td
                className={`${match.scoreA > match.scoreB ? "font-bold" : ""}`}
              >
              ...
              <td
                className={`${match.scoreB > match.scoreA ? "font-bold" : ""}`}
              >
      ...
      {sessionData?.user?.isAdmin && (
        <div className="mt-4">
          <hr />
          <h4 className="text-bold text-center text-xl">Add a new match</h4>
